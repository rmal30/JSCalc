...
function remcol(){
	for(var r = 1; r <= num_row; r++){ document.getElementById('cell' + ((r*num_col)-1).toString()).id = "-1";}
	for(var r = 1; r<num_row; r++){	
		for(var c = 0; c<num_col-1; c++){document.getElementById('cell' + ((r*num_col)+c).toString()).id = "cell" + ((r*num_col)+c-r).toString();}
		}
	num_col--;
...
for(var r = 0; r<Math.abs(diff_rows); r++){ if(diff_rows>0){addrow();}else{remrow();}}
for(var c = 0; c<Math.abs(diff_cols); c++){ if(diff_cols>0){addcol();}else{remcol();}}
update();
}
...
function move(e){
	try{
		switch (e.keyCode){
			...
			case 37: document.getElementById("cell" + (parseInt(document.activeElement.id.substring(4,document.activeElement.id.length))-1).toString()).focus(); break; 
			case 38: document.getElementById("cell" + (parseInt(document.activeElement.id.substring(4,document.activeElement.id.length))-num_col).toString()).focus(); break;
			case 39: document.getElementById("cell" + (parseInt(document.activeElement.id.substring(4,document.activeElement.id.length))+1).toString()).focus(); break;
			case 40: document.getElementById("cell" + (parseInt(document.activeElement.id.substring(4,document.activeElement.id.length))+num_col).toString()).focus();; break;
		}
	}catch(err){}
}
...
function open(file){
	reader.open('get', 'http://localhost:8000/' + file + "?" + Math.random(), true);
		reader.onreadystatechange = 
				function(){
					if(reader.status==404){window.location.reload();}
					else{
						html_table = "";
						cell_id=0;
...
function update(){
	...
	for(var i=0; i<num_row; i++){
		...
		for(var j=0; j<num_col; j++){ 
			...
			try{
				if(document.getElementById("cell" + (i*num_col + j).toString()).value[0]=="="){
					html_table = html_table + "<td>" + "<textarea rows=\"1\" type = \"text\" style = \"border:0px; width:60px;\" id = cell" + (i*num_col + j).toString() +">" + evaluate(document.getElementById("cell" + (i*num_col + j)).value, (i*num_col + j)) +"</textarea>" + "</td>" + "\n";
				}else if(eval(document.getElementById("cell" + (i*num_col + j).toString()).value)!==undefined){
					html_table = html_table + "<td>" + "<textarea rows=\"1\" type = \"text\" style = \"border:0px; width:60px;\" id = cell" + (i*num_col + j).toString() +">" + eval(document.getElementById("cell" + (i*num_col + j)).value, (i*num_col + j)) +"</textarea>" + "</td>" + "\n";
				}
				else {
					html_table = html_table + "<td>" + "<textarea rows=\"1\" type = \"text\"  style = \"border:0px; width:60px;\" id = cell" + (i*num_col + j).toString() +">" + document.getElementById("cell" + (i*num_col + j)).value +"</textarea>" + "</td>" + "\n";
					}
			}catch(err){
			html_table = html_table + "<td>" + "<textarea rows=\"1\" type = \"text\" style = \"border:0px; width:60px;\" id = cell" + (i*num_col + j).toString() +">" + document.getElementById("cell" + (i*num_col + j)).value +"</textarea>" + "</td>" + "\n";
			}
...
document.getElementById('cell' + ci.toString()).addEventListener('click', 
	...
	document.getElementById('cell' + ci.toString()).addEventListener('mousemove', 
		function(){
			//console.log(drag_id)
				for(var c = 0; c<num_col; c++){document.getElementById('cell' + ((Math.floor((ci)/num_col)*num_col) + c).toString()).style.height = document.getElementById('cell' + ci.toString()).style.height;}
				for(var r = 0; r<num_row; r++){document.getElementById('cell' + (((ci)%num_col) + r*num_col).toString()).style.width= document.getElementById('cell' + ci.toString()).style.width;}
...
document.getElementById('cellref').addEventListener('change', function(){
	...
	}catch(err){ document.getElementById('cell' + (letters2.indexOf(cellref[0].toUpperCase()) + ((parseInt(cellref.substring(1,cellref.length))-1)*num_col)).toString()).focus();}});
